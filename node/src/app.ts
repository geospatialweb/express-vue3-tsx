...
const startServer = (): void => {
  ...
  http
    ...
    .listen(Number(process.env.PORT), (): void => {
      ...
      logService.consoleLog(`HTTP server listening at ${process.env.HOST}:${process.env.PORT}`, blue)
    })
}
// PRODUCTION
// const startServer = (): void => {
...
//       const blue = '\x1b[34m%s\x1b[0m'
//       const logService = Container.get(LogService)
//       logService.consoleLog(`HTTPS server listening at ${process.env.HOST}:${process.env.PORT}`, blue)
//     })
// }
setPoolConnection()
startServer()
